...
formats = [
    ...
    """
            ...
            "questions": ["Question de la étape 1","Question de la étape 2",...,"Question de la étape n (selon nombre d'étape extremement détaillé pour détecter ou est l'erreur pour résoudre)"],
            "correct_answers": ["Réponse à l'étape 1","Réponse à l'étape 2",...,"n"],
            }
            """,
]
...
def format_step_by_step_question(raw_data):
    ...
    if isinstance(raw_data, str):  # est-ce que raw_data c'est une chaine de caractere?
        try:
            raw_data = json.loads(
                raw_data
            )  # si oui on le convertit en dict (json -> objet python)
        except json.JSONDecodeError:
            raise ValueError("Les données fournies ne sont pas un JSON valide.")
    # Vérification que le résultat final est bien un dict
    if not isinstance(
        raw_data, dict
    ):  # meme si raw_data n'est pas une chaine, ca peut etre un dictionnaire, car apres on va extraire de raw_data avec .get() et ca fonctionne que sur un dict
        raise ValueError("Les données doivent être un dictionnaire ou un JSON valide.")

    # Extractions des champs IMPORTANTS
    statement = raw_data.get(
        "énoncé", ""
    ).strip()  # ca signifie qu'il va chercher la valeur associée à la clé statement et si elle existe pas ca prend une chaine vide "" à la place, puis .strip() enleve les espaces inutiles
    questions = raw_data.get(
        "questions", []
    )  # meme principe ici, si jamais la clé question existe pas on prend une liste vide
    ...
    ):  # question DOIT êtr une liste et ne doit pas être vide, si l'un OU l'autre alors erreur
        raise ValueError("Il doit y avoir au moins une question d'étape.")

    # Vérification liste des réponses
    if (
        not isinstance(correct_answers, list) or len(correct_answers) == 0
    ):  # meme logique que pour les questions
    ...
    ):  # on vérif que ALL (tous) les elements verifient : d'abord que chaque question est une chaine de  caracteres, puis qu'elle n'est pas vide ou remplie d'espaces avec strip
...
abon = {
    "énoncé": "Dans un triangle rectangle ABC, rectangle en A, on sait que AB = 6 cm et AC = 8 cm. Détermine la longueur BC, puis calcule cos(B), sin(B) et tan(B).",
    "questions": [
